...
profesorRouter.get('/:id_p', (req, res) => {
    const id_p = parseInt(req.params.id_p);
    profesorController.getOne(id_p, (err, result) => {
        if (err) {
            return res.status(500).json({ message: err.message });
        }
        res.status(result.statusCode).json(result);
...
profesorRouter.put('/', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const newProfesor = req.body;
    profesorController.update(newProfesor, (err, result) => {
        if (err) {
            return res.status(500).json({ 'message': err.message });
        }
        res.status(result.statusCode).json(result);
...
profesorRouter.delete('/:id_p', (req, res) => {
    const id_p = parseInt(req.params.id_p);
    profesorController.delite(id_p, (err, result) => {
        if (err) {
            return res.status(500).json({ message: err.message });
        }
        res.status(result.statusCode).json(result);
