...
estudianteRouter.get('/:cod_e', (req, res) => {
    const cod_e = parseInt(req.params.cod_e);
    estudianteController.getOne(cod_e, (err, result) => {
        if (err) {
            return res.status(500).json({ message: err.message });
        }
        res.status(result.statusCode).json(result);
...
estudianteRouter.put('/', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const newEstudiante = req.body;
    estudianteController.update(newEstudiante, (err, result) => {
        if (err) {
            return res.status(500).json({ 'message': err.message });
        }
        res.status(result.statusCode).json(result);
